...
class ImageBinarizerApp:
    def __init__(self, master):
        ...
        title_font = (self.get_system_font(), 24, "bold")
        button_font = (self.get_system_font(), 14, "bold")
        ...
        top_centering_frame = ctk.CTkFrame(top_frame, fg_color="transparent")
        top_centering_frame.pack(fill=ctk.BOTH, expand=True)

        input_frame = ctk.CTkFrame(top_centering_frame)
        input_frame.pack(side=ctk.LEFT, expand=True)
        ...
        histogram_frame = ctk.CTkFrame(top_centering_frame)
        histogram_frame.pack(side=ctk.LEFT, expand=True)
        ...
        bottom_frame.pack(fill=ctk.BOTH, expand=True)

        bottom_centering_frame = ctk.CTkFrame(bottom_frame, fg_color="transparent")
        bottom_centering_frame.pack(fill=ctk.BOTH, expand=True)

        output_image_frame = ctk.CTkFrame(bottom_centering_frame)
        output_image_frame.pack(side=ctk.LEFT, expand=True)
        ...
        output_histogram_frame = ctk.CTkFrame(bottom_centering_frame)
        output_histogram_frame.pack(side=ctk.LEFT, expand=True)
    ...
    def draw_histogram(self, image, canvas, is_binary=False):
        if image is not None:
            ...
            canvas.create_text(canvas_width // 2, canvas_height - 5,
                               text=lang['brightness_level'],
                               fill="black",
                               font=(self.get_system_font(), 10, "bold"))  # Updated this line
            canvas.create_text(15, canvas_height // 2,
                               text=lang['pixel_count'],
                               fill="black",
                               angle=90,
